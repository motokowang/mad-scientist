...
class NeptuneApex:
    ...
    def __init__(self, url: LiteralString):
        ...
        self.__d__ = dict()
        self.__d__['datetime_epoch'] = self.json['istat']['date']
        self.__d__['datetime_tz'] = self.json['istat']['timezone']
        for json_line in self.json['istat']['inputs']:
            self.__d__[json_line['name']] = json_line['value']

    def sensors(self) -> dict:
        '''Wrapper for dict of sensors

        Returns:
            dict: key-value pair of (sensor name, sensor value)
        '''
...
def generate(stream_name, kinesis_client, output=True):
    apex = NeptuneApex('http://10.10.100.11/cgi-bin/status.json')
    
    while True:
        sleep(15)
        
        if output:
            print(*apex.sensors().items())

        kinesis_client.put_record(
            StreamName=stream_name,
            Data=json.dumps(apex.sensors()),
